...
DistReads=  P60vP7_zic %>% 
  ...
  labs(x = "Mean Zic ChIP Normalized Reads", y = "Number of Peaks")
DistReads
```
...
lossGain = P60vP7_zic %>% 
  ...
  labs(y = "# Peaks Mean \nSignal < 10 at P7 or P60", x ="Change")
lossGain
...
tab %>% 
 ...
 zic_motif = plt_dat %>% 
  ...
  theme_classic() +
  theme( panel.grid = element_blank(), legend.position = "bottom", legend.key.size = unit(.3, 'cm'))


# Histograms of Zic1 and Zic2 sites in Early, late and Staic peaks

zic_motif_hist =plt_dat %>% 
  dplyr::select(sequence_name, zic_sig, motifs, n_motifs_in_peak) %>% 
  distinct() %>% 
  dplyr::filter(!(motifs == "No Zic Motif")) %>% 
  ggplot(aes(x = n_motifs_in_peak)) + 
  geom_histogram(fill = "white", color = "black", bins = 6) +
  xlim(0,5)+
  facet_grid(motifs~zic_sig, labeller = labeller(zic_sig = c("DOWN" = "Early", "UP" = "Late", "N.S." = "Static")))+
  theme_classic()+
  labs(x = "Number of Motifs in Peak", y = "Number of Peaks")
...
zic_overlap = plt_dat %>% 
  ...
  labs( x = "Differential Zic Region", y= "# of Peaks", fill = "Overlap") +
  theme_classic()+
...
#  fig1 = plot_grid(
#   plot_grid(zic_ma,DistReads, lossGain,zic_motif, rel_widths = c(.3,.35 ,.15, .2), nrow = 1, labels = c("A","B", "C", "D")),
#   plot_grid(zic_motif_hist,zic_overlap, zic_loci2 ,rel_widths = c(.3, .4, .3), nrow = 1, labels = c( "E", "F", "G")),
#           ncol =1
# )

fig1 = plot_grid(
  plot_grid(zic_ma, lossGain, lg_examples, rel_widths = c(.35,.15 ,.5), nrow = 1, labels = c("A","B", "C")),
  plot_grid(zic_motif,zic_overlap, zic_loci2 ,rel_widths = c(.25, .5, .25), nrow = 1, labels = c( "D", "E", "F")),
          ncol =1
)
 
 
png("../figures/figure1.png", height = 8, width = 15, units = "in", res = 300)
fig1
dev.off()
