...
tf_b_1 = dev_bart %>% 
  ...
  dplyr::mutate(gene_sig = case_when(log2FoldChange > 1 & padj < 0.05 ~ "DIV7",
                log2FoldChange < -1 & padj < 0.05 ~ "DIV3",
                is.na(padj) ~ "filtered",
                TRUE ~ "Static")) %>% 
...
supp_gene_dis = plot_grid( plot_grid(tf_b_1, tf_b_2, tf_b_3, nrow =1, labels = c("A", "B", "C")),
           plot_grid(rrho_b1, rrho_b2, rrho_b1, nrow =1, labels = c("D", "E", "F")),
           plot_grid(b3,b2, b1, nrow = 1, labels = c("G", "H", "I")),
           ncol = 1,
           rel_heights = c(.55, .15, .3)
)
