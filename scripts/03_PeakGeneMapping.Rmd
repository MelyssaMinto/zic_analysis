...
```{r zic ratio fig.height=8, fig.width=12}
...
dist_prox = zic_loop_data %>% dplyr::rename(sequence_name = zic_peak) %>% 
  ...
  ungroup() %>% 
  left_join(n_peaks_to_gene)
```

```{r zic_ratio fig.height=8, fig.width=12}
test_list  = n_peaks_to_gene %>% 
  top_n(10, n) %>% 
   pull(gene_name)

plot_zic_ratio(test_list)$p2

 dist_prox %>%   
   dplyr::filter(!is.na(gene_sig)) %>% 
   group_by(anchor_type, gene_sig, zic_sig) %>% 
   summarize(mean_count= mean(count),
             mean_ratio = mean(ratio),
             sd_count = sd(count),
             sd_ratio = sd(ratio)) %>% 
   ungroup() %>% 
   ggplot(aes(x = zic_sig, y = mean_count,ymin = mean_count - sd_count, ymax = mean_count + sd_count, fill = anchor_type)) +
  ...
  dist_prox %>%   
   ...
   summarize(mean_count= mean(count),
             ...
             sd_ratio = sd(ratio)) %>% 
   ungroup() %>% 
   ggplot(aes(x = zic_sig, y = mean_ratio,ymin = mean_ratio - sd_ratio, ymax = mean_ratio + sd_ratio, fill = anchor_type)) +
  geom_errorbar(position = position_dodge2(width = 0.9)) +
      geom_col(position = position_dodge2(width = 0.9)) + 
    theme_classic() +
    scale_fill_manual(values = c( "grey", "black")) +
    scale_x_discrete(labels = c("DOWN" = "P7","N.S."= "Static","UP" = "P60"))+
    facet_wrap(~gene_sig, labeller = labeller(gene_sig = c("DOWN" = "Down-regulated","N.S."= "Constituitive","UP" = "Up-regulated") )) +
    labs(x = "Differential Zic Region", y = "prop. of Peaks", fill = "") +
    theme(strip.background = element_rect( fill = "white"),
          strip.text = element_markdown(),
          strip.text.y  = element_markdown(),
          strip.text.x  = element_markdown(),
