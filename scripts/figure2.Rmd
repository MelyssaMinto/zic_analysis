...
# > Source: Reddy et al. 2021 https://www.nature.com/articles/s41467-021-25846-3
ep_file <-read_delim("../sequencing_data/Reddy/E-P-Gene_map.txt",
                     delim = "\t", escape_double = FALSE, col_names = TRUE, trim_ws = TRUE) %>% 
          dplyr::select(starts_with(c("enhancer", "promoter")))
```
...
sa1 = mapped_data %>% 
  ...
  ggplot(aes(x = gene_sig, y = n, label = n, fill =  in_anchor))+
  geom_col() +
...
sb1 = mapped_data %>% 
  ...
  group_by(in_anchor) %>% 
  dplyr::count(zic_sig) %>% 
  ggplot(aes(x = zic_sig, y = n, label = n, fill =  in_anchor))+
  geom_col() +
  geom_text(vjust = 2, fontface = "bold", color = "black", size = 3)+
  #facet_wrap(~in_anchor, labeller = labeller(in_anchor = c("FALSE" = "Not in Anchor", "TRUE" = "With in Anchor"))) +
...
sc1 = mapped_data %>% 
  ...
  ggplot(aes(x = dnase_sig, y = n, label = n, fill =  in_anchor))+
  geom_col() +
  geom_text(vjust = 2, fontface = "bold", color = "black", size = 3)+
  #facet_wrap(~in_anchor, labeller = labeller(in_anchor = c("FALSE" = "Not in Anchor", "TRUE" = "With in Anchor"))) +
...
sd1 = mapped_data %>% 
  ...
  ggplot(aes(x = k27ac_sig, y = n, label = n, fill =  in_anchor))+
  geom_col() +
  geom_text(vjust = 2, fontface = "bold", color = "black", size = 3)+
  #facet_wrap(~in_anchor, labeller = labeller(in_anchor = c("FALSE" = "Not in Anchor", "TRUE" = "With in Anchor"))) +
...
#png("../figures/supp_figure2.png", height = 5, width = 7, units = "in", res = 300)
plot_grid(sa1,sb1,sc1,sd1, nrow = 2, labels = LETTERS[1:4])
#dev.off()
...
```{r supp fig 2, fig.height=5, fig.width=7}
...
sa2 = n_peaks_to_gene %>% 
  ...
  scale_fill_manual( name = "Gene Diff Signal", labels = c("Constituitive", "Down-regulated", "Up-regulated"), values = c("black", "blue", "red"))+
  theme_classic() +
...
```{r}
ratio_data = mapped_data %>% 
  dplyr::filter( zic_sig %in% c("UP", "DOWN", "N.S.")) %>% 
  dplyr::filter( gene_sig %in% c("UP", "DOWN", "N.S.")) %>% 
  dplyr::select(starts_with(c("zic", "gene"))) %>% 
  distinct() %>% 
  group_by(gene_name) %>% 
  add_count(zic_sig) %>% 
  ungroup() %>% 
  dplyr::select(gene_name, gene_sig, zic_sig, n) %>% 
  distinct() %>% 
  pivot_wider(id_cols = c("gene_name", "gene_sig"), values_from = "n", names_from="zic_sig", values_fill = 0) %>% 
  dplyr::mutate(p7_p60_zic = ifelse(UP >=1 & DOWN>=1, T,F),
                UP_ratio = UP/(UP+DOWN)) 
  #dplyr::filter(p7_p60_zic == TRUE) 

ks.ts <- ks.test(ratio_data$UP_ratio[ratio_data$gene_sig == "UP"], ratio_data$UP_ratio[ratio_data$gene_sig == "DOWN"], alternative = "two.sided")


ratio_data %>% 
  ggplot(aes(x = UP_ratio, color = gene_sig)) +
  stat_ecdf()+
  scale_color_manual(limits = c("DOWN", "UP", "N.S"), values = c("blue", "red", "black")) +
  labs(x = "P60:P7 Peak Ratio", y = "CDF", color = "Gene\nExpression")+
  theme_classic()

table(ratio_data$p7_p60_zic)

```

```{r}

# getting postional information for genes
genes = mapped_data %>% 
  dplyr::select(gene_name) %>% 
  distinct() %>% 
  pull(gene_name)

id = mapIds(org.Mm.eg.db, keys =genes , column ="ENTREZID", keytype="SYMBOL")
id = data.frame(GENEID = unlist(id), stringsAsFactors = F) %>% 
   dplyr::mutate(gene_name = rownames(.))

info = biomaRt::select(TxDb.Mmusculus.UCSC.mm10.knownGene, keys = id$GENEID  , columns=c("TXNAME", "TXSTART", "TXEND", "TXCHROM"), keytype="GENEID")
...
# calculating distance between peaks and genes 
zic_distance = mapped_data %>% 
  dplyr::select(starts_with(c("zic", "gene"))) %>% 
  distinct() %>% 
  left_join(gene_info %>% dplyr::select(-TXNAME) %>%  distinct()) %>% 
  dplyr::mutate(zic_chr = str_extract(zic_peak, "[^:]+"),
                zic_start = str_extract(zic_peak, "(?<=:).+(?=-)"),
                zic_end = str_extract(zic_peak, "\\b\\w+$"),
                zic_center = as.numeric(zic_start) + (as.numeric(zic_start) - as.numeric(zic_end))/2,
                distanceToTSS = abs(TXSTART - zic_center)) 


zic_distance %>% 
  dplyr::filter( zic_sig %in% c("UP", "DOWN", "N.S.")) %>% 
  dplyr::filter( gene_sig %in% c("UP", "DOWN", "N.S.")) %>% 
  ggplot(aes(x = distanceToTSS, color = gene_sig)) +
  geom_density() +
 xlim(0,10000) +
  theme_classic() +
    scale_color_manual(limits = c("DOWN", "UP", "N.S"), values = c("blue", "red", "black"))
...
zic_distance %>% 
  ...
  theme_classic() +
    scale_color_manual(limits = c("DOWN", "UP", "N.S"), values = c("blue", "red", "black"))+
  facet_zoom(xlim = c(0,1000), ylim = c(0,0.10), horizontal = F)
...
```{r panel c, fig.height=12, fig.width=7}
...
png("../figures/fig2_d.png", units = "in", height = 6, width = 15, res = 300)
plot_track("Nr4a3", tracks,  expand_l = 600000, expand_r = 5000, ylim = c(0,2))
dev.off()

d = rasterGrob(readPNG("../figures/fig2_d.png"))

```
![](../figures/fig2_d.png)
```{r figure 2, fig.height=12, fig.width=10}

fig2 = plot_grid(plot_grid(plot_grid(b, a1, ncol = 1, labels = c("A", "B")), 
                    c, ncol = 2,labels = c("", "C"), rel_widths = c(1,3)),
          d, ncol = 1, rel_heights =  c(2,1), labels = c("", "D"), vjust = 0)

png("../figures/figure2.png", height = 12, width = 10, res = 300, units = "in")
fig2
dev.off()
